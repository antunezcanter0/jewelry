<template>
  <div>
    ...
    <div v-if="!views_ignore.includes(this.$route.name)">
      <v-divider></v-divider>
...
export default {
  ...
  data() {
    return {
      ...
      dialog: { show: false },
      views_ignore: ['list_items', 'start_shopping', 'home']
    };
  },
  ...
  methods: {
    ...
    start() {
      ...
      axios
        .get(`${this.app.url}/mas_vendidos`)
        .then((response) => {
          let data = [];

          for (let i of response.data) {
            data.push(i.id_articulo);
          }

          axios({
            method: "get",
            url: `${this.app.url}/ariculos_en`,
            params: { data },
          })
            .then((response) => {
