  ...
  <div>
    ...
    >
      ...
      <v-col md="3" v-for="item in articulo.items" :key="item.id">
        <v-card outlined class="align-end fill-height">
              ...
              <v-list-item-content>
                <v-list-item-title>
                  <div v-if="item.precio_venta">
                    <span
                      v-if="item.en_oferta"
                      class="font-weight-black primary_gold--text body-1"
                    >
                    ...
                    <span
                      :class="
                        !item.en_oferta
                          ? 'font-weight-black primary_gold--text body-1'
    ...
    <v-divider></v-divider>
    <page-headers :text="`${$t('items_in_offer')}`"></page-headers>
    <alert :message="`${$t('message2')}`"></alert>
    <offers></offers>

    <v-divider></v-divider>
    <page-headers :text="`${$t('most_desired')}`"></page-headers>
    <alert :message="`${$t('message1')}`"></alert>
...
import MostDesired from './MostDesired.vue';

export default {
  name: "ListItems",

  components: { AddCartDialog, Alert, Offers, PageHeaders, MostDesired },
  ...
  methods: {
    ...
    start() {
      ...
      const params = { id_producto_categoria: this.id };
      axios
        .get(`${this.app.url}/articulo_x_producto`, { params })
        .then((response) => {
